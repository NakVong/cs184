...
Ray Camera::generate_ray(double x, double y) const {
  ...
  //
    Vector3D bottomLeft(-tan(0.5 * radians(hFov)), -tan(0.5 * radians(vFov)), -1);
    Vector3D topRight(tan(0.5 * radians(hFov)), tan(0.5 * radians(vFov)), -1);

    Vector3D cameraSpace((1 - x) * bottomLeft.x + x * topRight.x, (1 - y) * bottomLeft.y + y * topRight.y, -1); // linear interpolation
    Vector3D worldSpace = c2w * cameraSpace;
    worldSpace.normalize();

    Ray ray = Ray(pos, worldSpace);
    ray.max_t = fClip;
    ray.min_t = nClip;
